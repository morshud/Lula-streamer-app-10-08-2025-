...
class CallService extends BaseService {
    ...
    constructor() {
        super("");
        this.#callLogsCollection = firestore().collection('callLogs');
    }
    ...
    async addCallLog(logData) {
        try {
            const docRef = await this.#callLogsCollection.add({
                ...logData,
    ...
    async updateCallLog(logId, updateData) {
        try {
            await this.#callLogsCollection.doc(logId).update({
                ...updateData,
    ...
    async getCallLogs(userId) {
        try {
            ...
            const callerSnapshot = await this.#callLogsCollection
                .where('callerId', '==', userId)
                .get();

            // Query for logs where the user is the receiver
            const receiverSnapshot = await this.#callLogsCollection
